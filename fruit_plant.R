...
mysecondplot <- ggplot(mydf, aes(x= height, color= c("black", "red"), size= age))+
  geom_point()
...
plants %>% distinct(flower_color)

colors <- c('yellow'=  '#FFBF00',
           'white'= '#f4f6f7' ,
           'orange '= '#f39c12',
           'red'= '#e74c3c', 
           'green'= '#27ae60')

one <- ggplot(data= plants, aes(flower_color, number_products, fill=flower_color))+
  geom_bar(stat = "identity")+
  scale_fill_manual(values= colors)+
  ggtitle("flower color and products")

ggsave("one.jpg", width = 8, height = 5)
...
ggplot(plants, aes(y= common_name, x= energy_kcal, fill= number_of_products))+
...
# mutate----
plants %>% mutate(common=Number_countries<20) %>% view()
